...
(() => {
  ...
  /* ---------------- login "pakai username aja" (tanpa verifikasi) ----------------
     Setiap username dipetakan ke akun internal dengan email & password yang
     diturunkan langsung dari username itu sendiri. Tidak ada rahasia yang perlu
     diingat pengguna — konsekuensinya siapa pun yang tahu username bisa masuk
     sebagai user itu. Ini pilihan sadar untuk penggunaan pribadi/uji coba. */
  ...
  $("#form-login").addEventListener("submit", async (e) => {
    ...
    if (!USERNAME_LOGIN_RE.test(username)) {
      formMsg("#login-msg", "Username 3-20 karakter, huruf kecil/angka/underscore saja.", "error");
      return;
    }
    const btn = $("#btn-login");
    setBusy(btn, true, "Masuk / Daftar");
    const { email, password } = usernameToCredentials(username);

    // 1) coba masuk seolah username ini sudah pernah dipakai
    const signInResult = await sb.auth.signInWithPassword({ email, password });

    if (signInResult.data.session) {
      setBusy(btn, false, "Masuk / Daftar");
      // onAuthStateChange menangani transisi ke app / onboarding
      return;
    }

    // 2) belum ada -> buat akun baru untuk username ini
    const signUpResult = await sb.auth.signUp({ email, password });
    setBusy(btn, false, "Masuk / Daftar");

    if (signUpResult.error) {
      formMsg("#login-msg", signUpResult.error.message, "error");
      return;
    }
    if (!signUpResult.data.session) {
      formMsg("#login-msg",
        "Project Supabase ini masih mewajibkan konfirmasi email, jadi akun baru tidak bisa langsung aktif. " +
        "Matikan opsi \"Confirm email\" di Supabase Dashboard → Authentication → Providers → Email supaya login pakai username ini bisa langsung jalan.",
        "error");
      return;
    }
    pendingUsername = username;
    // onAuthStateChange akan lanjut ke onboarding karena profil belum ada
  });
  ...
  $("#form-onboarding").addEventListener("submit", async (e) => {
    ...
    if (!pendingUsername) { formMsg("#onboarding-msg", "Username tidak ditemukan, coba masuk ulang.", "error"); return; }
  ...
  async function loadOrCreateProfileFlow(user) {
    ...
    if (!profile) {
      ...
      $("#onboarding-username-display").textContent = "@" + (pendingUsername || "");
